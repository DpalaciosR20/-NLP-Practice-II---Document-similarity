...
def scrape_pubmed(num_articles: int = 300) -> list:
    ...
    """
    logging.info("Iniciando scraping de PubMed con Playwright (usando XPaths absolutos)...")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            while len(all_articles_data) < num_articles:
                logging.info(f"Recolectados {len(all_articles_data)} de {num_articles}. Buscando artículos en la página...")

                # Esperar y contar artículos
                page.wait_for_selector('a.docsum-title')
                article_count = page.locator('a.docsum-title').count()
